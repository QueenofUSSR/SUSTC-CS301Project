...
void moveForward(TIM_HandleTypeDef *htim)
{
	...
	HAL_GPIO_WritePin(RIGHT_MOTOR_GO_GPIO_Port, RIGHT_MOTOR_GO_Pin, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(RIGHT2_MOTOR_GO_GPIO_Port, RIGHT2_MOTOR_GO_Pin, GPIO_PIN_RESET);
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_1, 300);//L1满速前进
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_2, 300);//L2满速前进
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_3, 7200);//R1满速前进
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_4, 7200);//L1满速前进
}
...
void moveTurnRight90(TIM_HandleTypeDef *htim)
{
	moveTurnRightslow(htim);
    HAL_Delay(1320);
    motorBreak();
}

void moveTurnLeft90(TIM_HandleTypeDef *htim)
{
	moveTurnLeftslow(htim);
    HAL_Delay(1320);
    motorBreak();
}

void moveTurnLeft180(TIM_HandleTypeDef *htim)
{
	moveTurnLeftslow(htim);
	HAL_Delay(2540);
	motorBreak();
...
void moveForwardOneGrid(TIM_HandleTypeDef *htim)
{
	moveForward(&htim4);
	HAL_Delay(1330);
	motorBreak();
